...
IceGate - движок озера данных наблюдаемости, который хранит логи, трейсы, метрики и события в таблицах Apache Iceberg с DataFusion в качестве движка запросов.
...
| Язык | Rust 1.92+ (2024 edition) | Безопасная по памяти, высокопроизводительная среда выполнения |
...
1. Сервис Maintain отслеживает размер WAL
2. При достижении порога читает файлы WAL
3. Объединяет и оптимизирует данные
4. Записывает новые файлы данных Iceberg
5. Фиксирует новый снапшот в каталоге
6. Удаляет обработанные файлы WAL
