...
IceGate использует файлы конфигурации YAML или TOML. Формат определяется автоматически по расширению файла (`.yaml`/`.yml` для YAML, `.toml` для TOML).
...
**Примечание:** Эндпоинты метаданных `/labels`, `/label_values` и `/series` всегда читают только из Iceberg, независимо от этой настройки.
